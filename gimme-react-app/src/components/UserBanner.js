...
function UserBanner(props) {
    ...
    useEffect(() => {
        if (ctx.currentUser && ctx.currentUser._id !== props.user._id) {
            const followingIndex = props.user.followers.indexOf(
                ctx.currentUser._id
            );
            const followedByIndex = props.user.following.indexOf(
                ctx.currentUser._id
            );
    ...
    const handleFollowToggle = async (event) => {
        if (props.authUserFunctions) {
            if (!isFollowing) {
                const result = await props.authUserFunctions.followUser(
                    props.user
                );
                if (!result.error) setIsFollowing(true);
            } else {
                const result = await props.authUserFunctions.unfollowUser(
                    props.user
                );
    ...
    return (
        ...
        >
                ...
                <div className={classes.followStats}>
                    <NavLink to={"../following"} tabIndex={"-1"}>
                        <p className={classes.followStats__stat}>
                            <span className={classes.followStats__stat__num}>
                                {props.user.following.length}
                            </span>
                            <span className={classes.followStats__stat__label}>
                                following
                            </span>
                        </p>
                    </NavLink>
            ...
            {props.showFollowStatus && displayFollowStatus && (
