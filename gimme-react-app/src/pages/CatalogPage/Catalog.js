...
function CatalogPage(props){
  ...
  const changeListName = async (list, listName) => {
    if(list._id && props.authListFunctions){
      const result = await props.authListFunctions.changeListName(list, listName)
  ...
  const changeListPrivacy = async (list, setting) => {
    if(list._id && props.authListFunctions){
      const result = await props.authListFunctions.changeListPrivacy(list, setting)
  ...
  const deleteList = async (list) => {
    if(list._id && props.authListFunctions){
      ...
      if(!result.error){
        navigate(`/${props.user.handle}/lists/all-items`, {replace: true})
        refreshLists()
      }
    }
  }
        ...
        <Routes>
          <Route path='/'>
            { // Route only available to owner of catalog
              props.isOwner &&
              <Route path='create-list' exact element={
                <CreateListView
              ...
              lists.map((list,i) => (
                <Route path={(list._id) ? `${list._id}` : 'all-items'} key={i} exact element={
                  <ItemTableWithCache
                    ...
                    onChangeListName={(newName)=>changeListName(list, newName)}
                    onChangeListPrivacy={(setting)=>changeListPrivacy(list, setting)}
            ...
            <Route path='' element={<Navigate to='all-items'/>} replace={true}/>
            <Route path='*' element={(isLoading || newList)
              ? <InvalidPage
                  heading={' '}
                  body={' '}
              />
              ...
              : <InvalidPage
                  ...
                  body={"The list you're looking for is not available. Please look elsewhere."}
              />
            } replace={true}/>
          </Route>
        </Routes>
