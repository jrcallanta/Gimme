...
function FollowPage(props) {
  ...
  const initializer = async (callback?) => {
    ...
    const followingResult = (props.user.following)
      ? await makePost(`/users/idList`, {idList: props.user.following, fromId: fromId}).then((res) => res.json())
      : []
    const followersResult = (props.user.followers)
      ? await makePost(`/users/idList`, {idList: props.user.followers, fromId: fromId}).then((res) => res.json())
      : []
    await Promise.all([followingResult, followersResult])

    console.log(followingResult, followersResult)

    const sortedFollowing = (fromId) ? putMutualsFirst(fromId, followingResult.userList) : followingResult.userList
    const sortedFollowers = (fromId) ? putMutualsFirst(fromId, followersResult.userList) : followersResult.userList
  ...
  const putMutualsFirst = (userId, list) => {
    ...
    list.map((user) => {
      ...
      if(user.following.indexOf(userId) >= 0){
        if(user.followers.indexOf(userId) >= 0) followers_followed.push(user)
        else followers_notFollowed.push(user)
      }

      // if current user is not followed by this user
      else {
        if(user.followers.indexOf(userId) >= 0) nonFollowers_followed.push(user)
    ...
    <div className={classes.FollowPage}>
      <div className={classes.heading}>
        ...
        <div className={classes.userTitle} onClick={handleBack}>
          <h2 className={classes.userTitle__username}>{props.user.name}</h2>
          <h2 className={classes.userTitle__userhandle}>@{props.user.handle}</h2>
        </div>
      </div>
          ...
          : (displayedList && displayedList.length > 0)
            ? <div className={classes.listView}>
              { displayedList.map((user) => (
                  <div key={user.handle} className={classes.listView__item}>
                    <UserBanner
                      user={user}
