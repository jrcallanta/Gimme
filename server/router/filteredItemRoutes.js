module.exports = (data) => {
  ...
  app.route('/users/:userId/items/filtered/')
    .post(async(req, res) => {
      ...
      if(!user) res.status(400).send({
        ...
        await Promise.all(listsQuery.map(async(list) => {
          await Promise.all(list.listItems.map(async(itemId) => {
  ...
  app.route('/users/handle/:userHandle/items/filtered/')
    .post(async(req, res) => {
      ...
      if(!userQuery.length) res.status(400).send({
        ...
        await Promise.all(publicListQuery.map(async (list) => {
          await Promise.all(list.listItems.map(async (itemId) => {
            const item = await data.Item.find({
              _id: itemId,
              tags: { $all: tags }
            ...
            if(item.length) allPublicItemsFiltered.push(item[0])
          }))
        }))
  ...
  app.route('/lists/:listId/items/filtered/')
    .post(async(req, res) => {
